...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "`n🔍 Checking SNS sandbox status..." -ForegroundColor Yellow
        aws sns get-sms-sandbox-account-status --region us-east-1
        
        Write-Host "`n📱 To verify phone numbers in sandbox mode:" -ForegroundColor Cyan
        Write-Host "aws sns create-sms-sandbox-phone-number --phone-number +1234567890" -ForegroundColor Gray
        
        Write-Host "`n🌐 To request production access:" -ForegroundColor Cyan
        Write-Host "Run: .\scripts\request-sns-production-access.ps1" -ForegroundColor Gray
        
        Write-Host "`n🧪 Test the new functionality:" -ForegroundColor Yellow
...
Write-Host "`n🎯 Key Improvements in Production Version:" -ForegroundColor Green
