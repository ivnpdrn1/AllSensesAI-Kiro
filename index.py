...
def simulate_complete_pipeline(body):
    ...
    try:
        ...
        # Step 5: SMS Dispatch
        sms_results = dispatch_emergency_sms(user_profile, location_data, distress_result, event_id)
...
def dispatch_emergency_sms(user_profile, location_data, distress_result, event_id):
    ...
    for contact in opted_contacts:
        try:
            ...
            logger.error(f"SMS failed for {contact['name']}: {str(e)}")
            results.append({
                'contactName': contact['name'],
                'phone': contact['phone'],
                'status': 'failed',
...
def send_emergency_sms(body):
    """
    Direct SMS sending (legacy compatibility)
    """
    try:
